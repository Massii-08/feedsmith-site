/* Feedsmith — motion layer. Vanilla, no build step, no dependency, no network.
   ...
   v4 — one moving image, looped without a seam.
   13. the forge band (double-buffered video)             .forge-band video  */
(function () {
  'use strict';
  ...
  /* ---------------------------------------------------------------
     13. THE FORGE BAND — a loop with no seam
     The native `loop` cuts: the browser holds the last frame while it
     seeks back to zero, and on a five-second clip that stutter arrives
     every five seconds, which is exactly often enough to be the only
     thing anyone looks at. So the element is doubled — a muted clone
     stacked on the same rectangle — and the two crossfade across the
     loop point. The seam is covered by the fade instead of being timed
     away, so it survives a frame drop.

     Three states, and each one degrades into the next without a hole:
     the video plays; if it will not, the poster stays; if there is no
     poster either, the slab .band already painted is what shows. A
     missing file is therefore not an error path — play() rejects, the
     rejection is caught, and nothing else happens. The clone is only
     ever built once the source has proved loadable, so a 404 costs one
     failed request rather than two.

     Under reduced motion nothing is played at all: the poster is the
     whole thing. And with no hook on the page — no .forge-band video —
     none of this runs.
     --------------------------------------------------------------- */
  function forgeBand() {
    var vids = $$('.forge-band video'), i;
    if (!vids.length) return;

    if (reduced) {
      for (i = 0; i < vids.length; i++) {
        try { vids[i].pause(); } catch (e) { /* ignored */ }
        vids[i].removeAttribute('autoplay');
      }
      return;
    }

    /* the overlap, and with it the shortest clip worth double-buffering:
       under 2.5x this the fade would still be running when the next one
       is due, so those hand back to the native loop instead */
    var FADE = 900;
    var pairs = [], io = null;

    function tryPlay(el) {
      var p;
      try { p = el.play(); } catch (e) { return; }   // some engines throw here
      if (p && typeof p.catch === 'function') {
        p.catch(function () { /* autoplay refused or file missing: poster stays */ });
      }
    }

    function smoothLoop(v) {
      var clone = null, front = v;
      var opa = 0, fading = false, raf = null, native = false, shown = false, dead = false;

      v.removeAttribute('loop');
      v.loop = false;
      v.addEventListener('timeupdate', onTime, { passive: true });
      v.addEventListener('ended', onEnded, { passive: true });
      v.addEventListener('error', die, { passive: true });
      v.addEventListener('loadedmetadata', function () {
        if (shown) build();
      }, { passive: true });

      function other() { return front === v ? clone : v; }

      function drop() {
        if (!clone) return;
        try { clone.pause(); } catch (e) { /* ignored */ }
      ...
      /* the source failed: stop asking it for anything, leave the poster */
      function die() {
        dead = true;
        if (raf !== null) { cancelAnimationFrame(raf); raf = null; }
        fading = false;
      ...
      /* built late and only once the duration is known and long enough:
         a broken source never spawns a second request for itself */
      function build() {
        if (clone || native || dead || !v.parentNode) return null;
        var d = v.duration;
        if (!d || !isFinite(d) || d <= 0) return null;
        if (d < FADE / 1000 * 2.5) { goNative(); return null; }
        clone = v.cloneNode(true);
        clone.removeAttribute('id');
        clone.className = v.className ? v.className + ' vclone' : 'vclone';
        clone.loop = false;
        clone.muted = true;
        ...
        clone.addEventListener('timeupdate', onTime, { passive: true });
        clone.addEventListener('ended', onEnded, { passive: true });
        clone.addEventListener('error', goNative, { passive: true });
        v.parentNode.appendChild(clone);
        try { clone.load(); } catch (e) { /* ignored */ }
        return clone;
      }

      /* duration unknown, clip too short, or the clone would not load:
         hand back to the browser's own loop rather than risk a freeze */
      function goNative() {
        if (native) return;
        native = true;
        if (raf !== null) { cancelAnimationFrame(raf); raf = null; }
        ...
        drop();
        v.loop = true;
        if (shown && !dead) tryPlay(v);
      }

      function onTime(e) {
        if (native || dead || fading || e.target !== front) return;
        var d = front.duration;
        if (!d || !isFinite(d) || d <= 0) return;
        if (d < FADE / 1000 * 2.5) { goNative(); return; }
        if (!clone && !build()) return;          // no second buffer yet: wait
        if (front.currentTime >= d - FADE / 1000) swap();
      }

      function onEnded(e) {
        if (native || dead || fading || e.target !== front) return;
        swap();                                  // net: the fade was missed
      }

      function swap() {
        var nxt = other() || build();
        if (!nxt) { goNative(); return; }
        fading = true;
        try { nxt.currentTime = 0; } catch (e) { /* ignored */ }
        tryPlay(nxt);

        var from = opa, to = nxt === clone ? 1 : 0;   // the clone is always on top
        var t0 = 0;
        ...
        raf = requestAnimationFrame(function frame(ts) {
          ...
          var k = (ts - t0) / FADE;
          if (k > 1) k = 1;
          opa = from + (to - from) * (k * k * (3 - 2 * k));
          if (clone) clone.style.opacity = String(opa);
          ...
          fading = false;
          opa = to;
          try { prev.pause(); prev.currentTime = 0; } catch (e) { /* ignored */ }
        });
      }

      function halt() {
        try { v.pause(); } catch (e) { /* ignored */ }
        if (clone) { try { clone.pause(); } catch (e) { /* ignored */ } }
      }

      return {
        el: v,
        play: function () {                      // on screen
          if (dead) return;
          shown = true;
          if (v.preload === 'none') v.preload = 'auto';
          build();
          tryPlay(front);
          if (fading && other()) tryPlay(other());
        },
        pause: function () { shown = false; halt(); },   // off screen
        hide: halt,                                      // tab hidden
        resume: function () { if (shown && !dead) this.play(); },
        stop: function () {                              // preference flipped
          if (raf !== null) { cancelAnimationFrame(raf); raf = null; }
          fading = false;
          shown = false;
          halt();
    ...
    for (i = 0; i < vids.length; i++) pairs.push(smoothLoop(vids[i]));

    if (!hasIO) {
      for (i = 0; i < pairs.length; i++) pairs[i].play();
    } else {
      io = new IntersectionObserver(function (entries) {
        entries.forEach(function (e) {
          for (var k = 0; k < pairs.length; k++) {
            if (pairs[k].el !== e.target) continue;
            if (e.isIntersecting && !doc.hidden) pairs[k].play();
            else pairs[k].pause();
          }
        });
      }, { threshold: 0.1 });
      for (i = 0; i < pairs.length; i++) io.observe(pairs[i].el);
    }

    doc.addEventListener('visibilitychange', function () {
      for (var k = 0; k < pairs.length; k++) {
        if (doc.hidden) pairs[k].hide(); else pairs[k].resume();
      }
    });

    stopMotion.push(function () {
      if (io) io.disconnect();
      for (var k = 0; k < pairs.length; k++) pairs[k].stop();
    });
  }
